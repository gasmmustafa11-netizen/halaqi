...
export const PostsView: React.FC<PostsViewProps> = ({
  ...
  useEffect(() => {
    ...
    const loadPosts = async () => {
      ...
      try {
        const result = await api.getUnifiedPostsFeed();

        if (cancelled) return;

        if (!result.success) {
          console.error('[UNIFIED FEED]', result.error);
          setPosts([]);
          setUserPosts([]);
          setLikedPosts({});
          return;
        ...
        const unifiedPosts: UnifiedPost[] = (result.posts || []).map(
          (post: any) => ({
            ...post,
            id: String(post.id),
            postType: post.postType === 'user' ? 'user' : 'salon',
        ...
        // Keep this state only for compatibility with the existing
        // direct-post notification/focus flow.
        setUserPosts(
          unifiedPosts
            .filter((post) => post.postType === 'user')
            .map((post) => post as UserPost)
        );
        ...
        console.error('[Load unified posts feed error]', error);
  ...
  useEffect(() => {
    ...
    const loadDirectPost = async () => {
      ...
      try {
        const result = await api.getUserPostById(selectedPostId);
  ...
  const handleToggleComments = async (post: UnifiedPost) => {
  ...
  const handleAddComment = async (post: UnifiedPost) => {
    if (!user) {
      openAuthModal();
      return;
    }
