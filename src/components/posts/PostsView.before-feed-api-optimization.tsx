...
export const PostsView: React.FC<PostsViewProps> = ({
  ...
  useEffect(() => {
    ...
    const loadPosts = async () => {
      ...
      try {
        const [salonResults, userPostsResult] = await Promise.all([
          Promise.allSettled(
  ...
  const handleLike = async (post: SalonPost) => {
    ...
    try {
      const result = await api.toggleUserPostLike(post.id);
