...
;
  ...
  async getUnifiedPostsFeed(): Promise<any[]> {
    try {
      const rows = await sql`
        SELECT
          sp.id,
  ...
  async togglePostLike(
    ...
    try {
      const postTable = postType === 'user' ? 'user_posts' : 'salon_posts';

      const posts = postType === 'user'
        ? await sql`
            SELECT id, user_id
            FROM user_posts
            WHERE id = ${postId}
      ...
      if (existing.length) {
        ...
        await sql`
          INSERT INTO post_likes
            (id, post_type, post_id, user_id, created_at)
          VALUES
            (${likeId}, ${postType}, ${postId}, ${requestingUser.id}, NOW())
        `;

        liked = true;

        // Notify the owner of a user post when someone likes it.
        // Do not notify the user when they like their own post.
        if (
          postType === 'user' &&
          postOwnerId &&
          postOwnerId !== requestingUser.id
        ) {
          await this.createNotification({
            userId: postOwnerId,
      ...
      if (!liked) {
        this.state.postLikes = this.state.postLikes.filter(
          (l) =>
            !(l.postId === postId && l.userId === requestingUser.id)
        );
      }
  ...
  async addPostComment(
    ...
    try {
      ...
      // A post can belong to either salon_posts or user_posts.
      const salonPosts = await sql`
        SELECT id, owner_id
        FROM salon_posts
      ...
      const userPosts = salonPosts.length
        ? []
        : await sql`
            SELECT id, user_id
            FROM user_posts
      ...
      if (!salonPosts.length && !userPosts.length) {
        return {
          success: false,
      ...
      const postType: 'salon' | 'user' = salonPosts.length ? 'salon' : 'user';

      // Owner of the post who should receive the notification.
      const postOwnerId =
        salonPosts.length
          ? salonPosts[0]?.owner_id
          : userPosts[0]?.user_id;
  ...
  async deletePostComment(
    ...
    try {
      ...
      // Owner of a user post can delete comments on their own post.
      if (
        !allowed &&
      ...
      // Approved salon owner can delete comments on their salon post.
      if (
        !allowed &&
        requestingUser.role === 'salon_owner' &&
