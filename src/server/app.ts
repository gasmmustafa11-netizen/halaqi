...
// F-05: server-side imageUrl validation. Allows only HTTPS URLs served from
// the Halaqi Supabase Storage public bucket (whitelist, not blacklist).
...
app.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (!isValidStorageImageUrl(imageUrl)) {
        return res.status(400).json({
          success: false,
...
app.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const feed = await db.getUnifiedPostsFeed(req.user?.id, { cursor, limit });
      ...
      let visible = Array.isArray(feed.posts) ? feed.posts : [];
      ...
      // Malformed cursor → 400 (never silently fall back to page 1).
      if (error?.cursorInvalid) {
        return res.status(400).json({
          success: false,
          posts: [],
          nextCursor: null,
          hasMore: false,
          error: 'Cursor غير صالح.',
        });
      }
...
app.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      // ── F-04: server-side image size check (before Buffer.from) ──
      const approxBytes = Math.floor((match[2].length * 3) / 4);
      if (approxBytes > 25 * 1024 * 1024) {
        return res.status(413).json({
          success: false,
          error: 'حجم الصورة كبير جداً. الحد الأقصى 25 ميجابايت.',
        });
      }
...
async function ensureConversationStateTable(): Promise<void> {
  if (!conversationStateReady) {
    conversationStateReady = (async () => {
      ...
      `;
      // Ensure state_json column exists (may be missing on older tables).
      await sql`ALTER TABLE conversation_states ADD COLUMN IF NOT EXISTS state_json TEXT`;
...
async function tryGroqFallback(args: {
  ...
  try {
    ...
    const histMsgs: any[] = [];
    if (Array.isArray(args.conversationHistory)) {
      for (const h of args.conversationHistory.slice(-6)) {
        const role = h.role === 'ai' ? 'assistant' : 'user';
        histMsgs.push({ role, content: h.text || h.message || '' });
      }
    }
    // Add context about selected salon/service/date/time from conversationState
    const cs = args.conversationState || {};
    // If the user is clearly asking to find/look up a (possibly different) salon,
    // or just greeting / chatting generally (short message with no booking intent),
    ...
    const isGenericChat =
      ...
      rawU.length < 4;
    let contextNote = '';
    if (!looksLikeNewSearch && !isGenericChat) {
      if (cs.salonId || cs.salonName) contextNote += `الصالون المختار: ${cs.salonName || cs.salonId}. `;
      if (cs.serviceName) contextNote += `الخدمة: ${cs.serviceName}. `;
      if (cs.date) contextNote += `التاريخ: ${cs.date}. `;
      if (cs.time) contextNote += `الساعة: ${cs.time}. `;
    }

    const firstUserMsg = contextNote
      ? `[معلومات السياق: ${contextNote}]\n${args.userText}`
      : args.userText;
    ...
    let choice1 = res1.choices?.[0]?.message as any;
    let cards = args.cards || [];
    let reply = '';
    const toolCallNames: string[] = [];
    let toolResults: any[] = [];

    // DIRECT BOOKING PATH: if the user explicitly confirmed and we have all
    // booking fields in state, bypass model autonomy and force create_booking.
    // This prevents Groq from drifting into get_availability/salons on confirm.
    if (
      args.explicitBookingConfirm &&
      (cs.salonId || cs.salonName) &&
      ...
      const bookingArgs: any = {
        salonId: cs.salonId || undefined,
        serviceId: cs.serviceId || undefined,
        serviceName: cs.serviceName || undefined,
        date: cs.date,
        timeSlot: cs.time,
        confirmed: true,
      };
      toolCallNames.push('create_booking');
      const allowBooking = true;
      console.log('[AI Salon] GROQ_DIRECT_BOOKING: salonId=' + cs.salonId + ', serviceName=' + cs.serviceName + ', serviceId=' + cs.serviceId + ', date=' + cs.date + ', time=' + cs.time + ', userId=' + (args.user?.id || 'none'));
      let resTool: any;
      try {
        resTool = await executeTool(
          'create_booking',
          bookingArgs,
          args.dbModule,
          sqlImport,
          { user: args.user, allowBooking, conversationState: cs }
        );
      } catch (bookingErr: any) {
        console.error('[AI Salon] GROQ_DIRECT_BOOKING error:', bookingErr?.message || bookingErr);
        resTool = { error: bookingErr?.message || 'خطأ غير متوقع أثناء الحجز.' };
      }
      toolResults.push({ name: 'create_booking', response: resTool });
      reply =
        (resTool && (resTool.message || resTool.error || (resTool.success ? 'تم تأكيد الحجز بنجاح! ✓' : null))) ||
        'تم إرسال طلب الحجز.';
      if (resTool && resTool.error) {
        reply = resTool.error;
      }
      console.log('[AI Salon] GROQ_FALLBACK: direct create_booking.', resTool && resTool.error ? 'Error: ' + resTool.error : 'OK');
      return { reply, cards, toolCalls: toolResults, reason: 'direct_booking' };
    }
    ...
    if (choice1?.tool_calls && choice1.tool_calls.length > 0) {
      ...
      for (const tc of choice1.tool_calls) {
        ...
        // Relaxed booking check: allow when user explicitly confirmed and all booking fields present.
        const allowBooking =
          !!args.user &&
          !!args.explicitBookingConfirm &&
          name === 'create_booking' &&
          ...
          !!argsParsed.date &&
          !!argsParsed.timeSlot &&
          (!!argsParsed.serviceId || !!argsParsed.serviceName) &&
        ...
        if (resTool.salons && resTool.salons.length > 0 && !resTool.blocked) {
      ...
      const res2 = await groq.chat.completions.create({
        messages: [
          ...messages2,
          { role: 'user', content: 'الإجابة النهائية بالعربية/العراقية فقط. اعتمد على نتائج الأدوات أعلاه. لا تستدعِ أي أدوات الآن ولا تخترع صالونات أو أسعاراً.' }
        ],
        model: 'openai/gpt-oss-120b',
        tools: openAITools as any,
        tool_choice: 'none',
...
// text, writing into the supplied resolvedState object. Runs for
// BOTH Gemini and Groq fallback so booking details always persist.
// ============================================================
function extractBookingDetailsFromUserText(userText: string | undefined, resolvedState: any): void {
...
app.post('/api/ai-salon', optionalAuthMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // If this turn is only a generic greeting/short chat (no search, no booking
    // detail, no service request), drop any stale booking fields loaded from the
    // persisted state so the model doesn't keep claiming an old salon/booking
    // (e.g. "لديك حجز في رويال") every time the user starts talking.
    const rawMsgTrim = String(message || userText || '').trim();
    const isGenericGreeting =
      /^\s*(سلام|هلا|اهلا|شلون|شخبار|صباح|مساء|السلام|هاي|اوكي|حسنا|توكل)\s*$/i.test(rawMsgTrim) ||
      (rawMsgTrim.length < 5 &&
        !/\d/.test(rawMsgTrim) &&
        !/صالون|ابحث|دور|خدمات|حجز|صبغ|حلاقة/.test(rawMsgTrim));
    ...
    const systemInstruction = `أنت مساعد صالونات ذكي ومحترف في العراق، تعمل من منصة Halaqi. مهمتك مساعدة المستخدمين في البحث عن صالونات وحجز مواعيد بأسلوب مهني وودود.

تعليمات عامة:
- استخدم بيانات Halaqi الحقيقية فقط من خلال الأدوات المتاحة. لا تخترع أي معلومة.
- لا تستخدم Regex أو تخمينات. اعتمد فقط على نتائج الأدوات.
- لا تقل "لا أعرف" أو تطلب توضيحاً إذا كان بإمكانك البحث عبر الأدوات.
- الرد بالعربية العراقية بأسلوب مختصر ومحترف.
...
حالة السياق الحالية المعتمدة من الخادم (هذه هي الحقيقة الوحيدة الموثوقة عن الصالون/الخدمة المختارة، ويجب أن تتجاوز أي ذكر قديم في سجل المحادثة): ${
      ...
      try {
        ...
        for (
          ...
          if (turn === 1) {
            ...
            // Preserve conversation history + tool turns across multi-turn tool calls.
            contents = [
              ...priorContents.slice(0, -1),
              initialUserContent,
              ...toolConversationParts,
            ];
          }

          console.log('[AI DEBUG] ===== GEMINI TURN ' + turn + ' START =====');
          console.log('[AI DEBUG] USER:', turnContent);
          console.log('[AI DEBUG] TOOLS_ENABLED: true');
          ...
          if (
            ...
            for (const call of calls) {
              ...
              // bookingMatchesConfirmedState: check that Gemini's booking args are
              // complete and consistent. We relaxed the strict equality check because
              // Gemini may resolve IDs differently. The create_booking tool validates
              // the actual salon/service existence against the DB.
              const bookingMatchesConfirmedState =
                name === 'create_booking' &&
                !!resolvedBookingArgs.salonId &&
                !!resolvedBookingArgs.date &&
                !!resolvedBookingArgs.timeSlot &&
                (!!resolvedBookingArgs.serviceId || !!resolvedBookingArgs.serviceName);
              ...
              if (res.salons && res.salons.length > 0 && name === 'search_salons' && !res.blocked) {
            ...
            modelCalledSearch = responses.some(
              (r: any) => r.name === 'search_salons' && r.response && r.response.salons && r.response.salons.length > 0
            );
            ...
            // Preserve the model tool-call content followed by the formal
            // function responses so the next Gemini turn can continue correctly.
            const modelContentForToolTurn =
              response?.candidates?.[0]?.content || null;

            if (modelContentForToolTurn) {
              toolConversationParts.push(modelContentForToolTurn);
            }

            toolConversationParts.push({
              role: 'user',
          ...
          if (rawText) {
            if (turn === 1) {
              ...
              // Try JSON parse first in case model returned structured response.
              try {
                const jm = rawText.match(/\{[\s\S]*\}/);
                if (jm) {
                  const jp = JSON.parse(jm[0]);
                  if (jp.reply) { reply = jp.reply; intent = jp.intent || intent; }
                  else { reply = rawText; }
                } else { reply = rawText; }
              } catch (e) { reply = rawText; }
            } else {
        ...
        // SERVER-SIDE BOOKING DETAIL EXTRACTOR (shared helper)
        ...
        if (!reply && turn >= 2) {
          if (cards.length) {
            ...
            if (!resolvedState.time) missing.push('الوقت');
            if (missing.length > 0) {
              reply = `تمام، الصالون "${resolvedState.salonName || 'المحدد'}" محفوظ. يرجى تحديد ${missing.join(' و')} لاستكمال الحجز.`;
            } else {
        ...
        // Ensure booking details still get extracted even when Gemini fails
        // (so the Groq fallback receives service/date/time context).
        extractBookingDetailsFromUserText(userText, resolvedState);
        if (process.env.GROQ_API_KEY) {
          ...
          // Extract salonId/salonName from Groq search results into resolvedState
          // so that subsequent turns know which salon is selected.
          if (cards.length > 0 && !resolvedState.salonId) {
            const firstCard = cards[0];
            if (firstCard.id) resolvedState.salonId = firstCard.id;
            if (firstCard.name && !resolvedState.salonName) resolvedState.salonName = firstCard.name;
          }
          console.log('[AI Salon] GROQ_FALLBACK tool calls:', (groqRes.toolCalls || []).map((t: any) => t.name).join(',') || 'none');
          modelCalledSearch = (groqRes.toolCalls || []).some((t: any) => t.name === 'search_salons') && cards.length > 0;
        } else {
    ...
    // ============================================================
    // SERVER-SIDE TOOL ORCHESTRATION FALLBACK
    // Runs whenever the model (Gemini or Groq) failed to call the
    // right tool or gave a generic/unhelpful reply. Ensures the
    // user gets real data for common intents:
    //   1) search request (name/city/area) with no salon -> search_salons
    //   2) "services" request with a selected salon -> get_salon_services
    //   3) "availability/حجز" with salon+date -> get_availability
    // ============================================================
    {
      const wantsServices = /^(\s*)(خدمات|الخدمات|شو عندك|شنو عندك|اسعار|الأسعار|قايمة|القائمه|قائمة|شو الخدمات|الخدمات المتوفرة|عر[جي] لي الخدمات)/iu.test(String(userText || '').trim());
      const { executeTool } = await import('../services/aiSalonTools');
      const sqlImport = async () => await import('./lib/pg-compliant');
      const orchContext = { user: req.user, conversationState: resolvedState };

      // --- 1) SEARCH orchestration: user asked for a salon by name/city and
      // the model did NOT surface real salons itself. This guards against the
      // model inventing/guessing a salon (e.g. always "رويال لاونج") or claiming
      // "not found" even when the salon exists. Runs even if a salon is already
      // selected so the user can switch to a DIFFERENT real salon.
      // A "new search" is recognized by an explicit search verb and/or the user
      // mentioning a salon name — but NOT by booking confirmations/cancellations,
      // service requests, or inline booking details.
      {
        ...
        if (
          ...
          try {
            const kw = searchTerm.replace(/(ابغى|اريد|أريد|عايز|دور|بس|لي|أعطيني|اعطيني|وين|شنو عندك|ابحث|ادور|وريد|ابغي|ابي|لقا|يلقى|شوف|عن)/g, ' ').trim();
            const sr = await executeTool('search_salons', { keyword: kw, location: '' }, dbModule, sqlImport, {
              user: req.user,
              // Deliberately omit salonId so search_salons is NOT blocked by the
              // "salon_already_selected" guard. This orchestration call only fires
            ...
            if (sr && sr.salons && sr.salons.length > 0) {
              ...
              reply = `وجدت لك هذه الصالونات في Halaqi:\n` +
                cards.map((c: any, i: number) => `${i + 1}. ${c.name}${c.area ? ' — ' + c.area : ''}`).join('\n') +
                `\n\nاختر صالوناً لأرى خدماته.`;
              // Starting a fresh salon search: clear stale booking fields from a
              // previously selected salon so the model/user isn't confused by old
              // service/date/time belonging to a different salon.
              resolvedState.serviceId = null;
              resolvedState.serviceName = null;
              resolvedState.date = null;
              resolvedState.time = null;
              if (cards.length === 1 && cards[0].id) {
                resolvedState.salonId = cards[0].id;
                resolvedState.salonName = cards[0].name;
              }
            }
          } catch (orchErr: any) {
            console.error('[AI Salon] orchestration search_salons error:', orchErr?.message || orchErr);
          }
        }
      }

      // --- 2) SERVICES orchestration: salon selected + user asked for services ---
      if (resolvedState.salonId && (wantsServices || /lacks services|empty|no data/i.test(reply || ''))) {
        try {
          const servicesRes = await executeTool(
            'get_salon_services',
            { salonId: resolvedState.salonId },
            dbModule,
            sqlImport,
            orchContext
          );
          if (servicesRes && servicesRes.services && servicesRes.services.length > 0) {
            const salonName = resolvedState.salonName || 'الصالون';
            const list = servicesRes.services
              .map((s: any) => `• ${s.name}${s.price ? ' — ' + s.price + ' د.ع' : ''}`)
              .join('\n');
            reply = `خدمات ${salonName}:\n${list}\n\nأي خدمة تريد حجزها؟ أرسل اسم الخدمة مع التاريخ والوقت.`;
          }
        } catch (orchErr: any) {
          console.error('[AI Salon] orchestration get_salon_services error:', orchErr?.message || orchErr);
        }
      }

      // --- 3) BOOKING-SUMMARY orchestration: the user just supplied all booking
      // details in this turn. If the model errored or returned garbage, always
      // answer with a server-built confirmation summary instead of "حدث خطأ".
      {
        const detTerm = String(userText || '').trim();
        const bookingComplete =
          resolvedState.salonId &&
          (resolvedState.serviceId || resolvedState.serviceName) &&
          resolvedState.date &&
          resolvedState.time;
        const userProvidedBookingDetails =
          /\d{1,2}[\/\-]\d{1,2}/.test(detTerm) ||
          /\d{1,2}:\d{2}/.test(detTerm) ||
          /صبغ|حلاقة|تشذيب|ماسك|كيراتين|حواجب/i.test(detTerm);
        const replyIsErrorOrEmpty =
          !reply || reply.length < 5 || /\bحدث خطأ\b|لا يستطيع|еrror|Failed/i.test(reply);

        if (bookingComplete && userProvidedBookingDetails && replyIsErrorOrEmpty) {
          reply =
            `تمام، ملخص الحجز المطلوب:\n` +
            `- الصالون: ${resolvedState.salonName || resolvedState.salonId}\n` +
            `- الخدمة: ${resolvedState.serviceName || resolvedState.serviceId}\n` +
            `- التاريخ: ${resolvedState.date}\n` +
            `- الوقت: ${resolvedState.time}\n\n` +
      ...
      // --- 4) LAST-RESORT: never surface a raw "حدث خطأ" to the user. If the
      ...
      if (!reply || /\bحدث خطأ\b|الاستجابة البديلة|Failed|temporary error/i.test(reply)) {
        if (changeSalonRequested) {
          ...
          reply = hasBooking
            ...
            : `الصالون "${resolvedState.salonName || resolvedState.salonId}" محدد. ماذا تريد أن تفعل به؟ (خدمات / مواعيد / حجز)`;
        } else {
    ...
    return res.json({
      ...
      conversationState: {
        ...
        // STATE MERGING: never overwrite a valid value with undefined/null/empty.
        // Only override when the incoming value is a non-empty string.
        salonId:
          (typeof entities?.salonId === 'string' && entities.salonId.trim())
            ? entities.salonId.trim()
            : resolvedState.salonId || undefined,

        salonName:
          (typeof entities?.salonName === 'string' && entities.salonName.trim())
            ? entities.salonName.trim()
            : resolvedState.salonName || undefined,

        serviceId:
          (typeof entities?.serviceId === 'string' && entities.serviceId.trim())
            ? entities.serviceId.trim()
            : resolvedState.serviceId || undefined,

        serviceName:
          (typeof entities?.serviceName === 'string' && entities.serviceName.trim())
            ? entities.serviceName.trim()
            : resolvedState.serviceName || undefined,

        date:
          (typeof entities?.date === 'string' && entities.date.trim())
            ? entities.date.trim()
            : resolvedState.date || undefined,

        time:
          (typeof entities?.time === 'string' && entities.time.trim())
            ? entities.time.trim()
            : resolvedState.time || undefined,
