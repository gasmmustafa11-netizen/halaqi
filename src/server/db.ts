...
// Ensure salon cover update column exists (additive, safe migration).
  ...
  async getUnifiedPostsFeed(
    viewerUserId?: string,
    opts?: { cursor?: string | null; limit?: number }
  ): Promise<{ posts: any[]; nextCursor: string | null; hasMore: boolean }> {
    try {
      const viewerId = viewerUserId || '';
      const limit = Math.min(Math.max(opts?.limit ?? 20, 1), 100);
      // cursor format: "created_at_raw|id" — created_at_raw is the lossless
      // TIMESTAMPTZ text produced by Postgres (full microsecond precision).
      let cursorCreatedAt: string | null = null;
      let cursorId: string | null = null;
      if (opts?.cursor) {
        const sep = opts.cursor.indexOf('|');
        if (sep <= 0 || sep === opts.cursor.length - 1) {
          // Malformed cursor: must contain 'date|id' with both non-empty.
          const err: any = new Error('INVALID_CURSOR');
          err.cursorInvalid = true;
          throw err;
        }
        const datePart = opts.cursor.slice(0, sep);
        const idPart = opts.cursor.slice(sep + 1);
        if (!datePart || !idPart || Number.isNaN(Date.parse(datePart))) {
          const err: any = new Error('INVALID_CURSOR');
          err.cursorInvalid = true;
          throw err;
        }
        cursorCreatedAt = datePart;
        cursorId = idPart;
      }
      ...
      const rows = await sql`
        SELECT
          ...
          sp.caption,
          sp.created_at,
          sp.created_at::text AS created_at_raw,
          sp.updated_at,
          sp.like_count,
          sp.comment_count,
          EXISTS(
        ...
        SELECT
          ...
          up.caption,
          up.created_at,
          up.created_at::text AS created_at_raw,
          up.updated_at,
          up.like_count,
          up.comment_count,
          EXISTS(
      ...
      // hasMore = the query fetched limit+1 rows; fired only when a full page exists.
      let hasMore = false;
      let fetchRows = Array.isArray(rows) ? rows as any[] : [];
      if (fetchRows.length > limit) {
        hasMore = true;
        fetchRows = fetchRows.slice(0, limit);
      }

      const posts: any[] = fetchRows.map((p: any) => ({
        id: String(p.id),
      ...
      // nextCursor is derived from the OLDEST chronological element in the page,
      // NOT from the score/diversity reorder. fetchRows is already in
      // created_at DESC / id DESC order, so the last element is the boundary.
      // created_at_raw is the lossless TIMESTAMPTZ text, preserving full
      // microsecond precision so the next page re-partitions identically.
      let nextCursor: string | null = null;
      if (hasMore && fetchRows.length > 0) {
        const boundary = fetchRows[fetchRows.length - 1];
        const rawTs = boundary.created_at_raw
          ? String(boundary.created_at_raw)
          : new Date(boundary.created_at).toISOString();
        nextCursor = `${rawTs}|${String(boundary.id)}`;
      }

      // Strip internal scoring fields before returning.
      const clean = ranked.map(({ score, interestMatch, sourceId, ...rest }: any) => rest);
      return { posts: clean, nextCursor, hasMore };
    } catch (error: any) {
      if (error?.cursorInvalid) throw error;
      console.error(
      ...
      if (idempotencyKey && payloadHash) {
        ...
        try {
          ...
          const existing = await client.query(
            `SELECT post_id, response_status, response_body, payload_hash
